...
if __name__ == '__main__':
    ...
    array = []
    array.append(elem_trs[15])
    for tr in array:
        elem_tds = tr.find_elements_by_xpath('.//td')
        print(elem_tds)
        print(elem_tds[0].get_attribute('outerHTML'))
        # 基金代码
        ...
        # 原因：第一个tr由于使用滚屏已经在浏览器屏幕的外面，所以里面的a标签无法执行click；
        # 第十五个tr内的a标签执行click操作则不报错，因为此时tr在浏览器屏幕内处于可见状态+可操作状态
